  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        (!isStatic && github) ? "cursor-pointer" : "cursor-default"
      )}
    ...
    >
      <div className={cn(
        ...
        (!isStatic && github) && "group-hover:border-primary/30"
      )}>
        <img 
          src={image} 
          ...
          className={cn(
            ...
            (!isStatic && github) && "group-hover:scale-105"
          )}
        ...
        />
        {(!isStatic && github) && <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />}
      </div>
      
      <div className="space-y-3 flex flex-col flex-grow">
        <h3 className={cn(
          "text-2xl font-display font-bold text-white transition-colors duration-300",
          (!isStatic && github) && "group-hover:text-primary"
        )}>
          {title}
          {(!isStatic && github) && <ArrowUpRight size={18} className="inline-block ml-2 opacity-0 -translate-y-1 translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 group-hover:translate-y-0 transition-all text-primary" />}
        </h3>
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((tag, i) => (
            <span key={i} className={cn(
              ...
              (!isStatic && github) && "group-hover:border-primary/20 group-hover:text-primary group-hover:bg-primary/5 uppercase tracking-widest"
            )}>
              {tag}
            </span>
          ))}
